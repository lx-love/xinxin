	...
	$ .PullToRefresh = $ .PullToRefresh.extend（{
		init：function（element，options）{
			...
			this.options = $ .extend（true，{
				下： {
					提示： {
						颜色：['008000'，'d8ad44'，'d00324'，'dc00b8'，'017efc']，
		...
		拉：功能（deltaY）{
			...
			//固定Android 4.1.x
			canvas.style.display ='none'; //从DOM分离
			canvas.offsetHeight; //强制分离
			canvas.style.display ='inherit'; //重新连接DOM
			this.canvasUtils.drawArcedArrow（ctx，size / 2 + 0.5，size / 2，size / 4，0 * Math.PI，5/3 * Math.PI * ratioPI，false，1,2,0.7853981633974483,25， options.down.tips.lineWidth，this.options.down.tips.lineWidth）;
		}，
		...
		canvasUtils：（function（）{
			...
			var drawArcedArrow = function（ctx，x，y，r，startangle，endangle，anticlockwise，style，which，angle，d，lineWidth，lineRatio）{
				'use strict';
				style = typeof（style）！='undefined'？风格：3;
				which = typeof（which）！='undefined'？其中：1;
				angle = typeof（angle）！='undefined'？角度：Math.PI / 8;
				...
				if（which＆2）{
					...
					drawArrow（ctx，sx-lineRatio * Math.sin（endangle），sy + lineRatio * Math.cos（endangle），destx-lineRatio * Math.sin（endangle），desty + lineRatio * Math.cos（endangle），style， 2，角度，d）
				}
				ctx.restore（）;
			}
			...
			var spinColors = function（currentIteration，totalIterations）{
				...
				oldStep = step;
				返回“rgb（”+ parseInt（r）+“，”+ parseInt（g）+“，”+ parseInt（b）+“）”;
			}
			...
			var spin = function（t）{
				...
				ctx.clearRect（0，0，size，size）;
				//固定Android 4.1.x
				canvasObj.style.display ='none'; //从DOM分离
				canvasObj.offsetHeight; //强制分离
				canvasObj.style.display ='inherit'; //重新连接DOM
				ctx.beginPath（）;
